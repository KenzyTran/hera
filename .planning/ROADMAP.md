...
**Wave 1** *(BLOCKING — closes Phase 3 SC#2 before observability/cleanup deploy against real traffic)*
- [ ] 04-01-PLAN.md — Agent protocol-bridge: agent/hera_agent/main.py adds POST /invocations static-envelope stub per AgentCore HTTP protocol contract (canonical awslabs sample shape — voice loop continues on /ws). Local docker rebuild + curl gate, ECR push, in-place cdk deploy hera-agentcore (version=2 → version=3), AgentCore data-plane invoke-agent-runtime smoke probe. RUNBOOK Phase 4 protocol-bridge deploy section. **Closes Phase 3 SC#2 (deferred from Plan 03-05).** Covers Phase 3 DEP-02 + DEM-02 live-loop portion. *(2-3 commits; 1 ECR push + 1 cdk deploy + 1 smoke probe live AWS work)*

**Wave 2** *(parallel after Wave 1; file-disjoint — observability touches infra/modules/observability/ + infra/envs/prod/{main,outputs}.tf + RUNBOOK Phase-4-observability heading; cleanup-verify touches bin/cleanup-verify.sh + RUNBOOK Phase-4-cleanup heading)*
- [ ] 04-02-PLAN.md — Observability module + dashboard + 3 alarms: new infra/modules/observability/ (versions/variables/main/outputs); CloudWatch dashboard hera-prod with 5 panels (active sessions, latency p50/p95, error rate, Bedrock invocations+tokens, billing); 2 operational alarms (error_rate >5%/5min via metric_query arithmetic, latency_p95 >5s/5min via extended_statistic); 1 billing alarm in us-east-1 via second provider alias (AWS/Billing service constraint). Zero new IAM (alarm_actions=[] per D-35; no SNS/email/Lambda). RUNBOOK Phase 4 observability walkthrough + OBS-04 (D-36 — no per-IP rate limit; AgentCore concurrency cap=2 is the gate) + OBS-05 (D-35 — no SNS hook; manual-stop fallback) trade-offs. Covers OBS-01, OBS-02, OBS-03, OBS-04 (documented), OBS-05 (documented). *(7-8 commits; 1 terraform apply live AWS work; ZERO Bedrock spend)*
- [ ] 04-03-PLAN.md — bin/cleanup-verify.sh + RUNBOOK Phase 4 cleanup quy trinh: bash + AWS CLI script per bin/verify-kb.sh pattern (set -euo pipefail; aws+jq preflight; HERA_REGION default ap-northeast-1) with 19 read-only resource checks covering KB / S3 Vectors index+bucket / source bucket / AgentCore Runtime / CDK CFn stack / 2 CW log groups / 4 IAM roles+1 policy / ECR repo / CloudFront / widget S3 / Lambda+FuncURL / billing alarm + 2 op alarms / dashboard. Verify-only (D-39 — operator destroys; script verifies); never calls Cost Explorer (D-38 — RUNBOOK 24h-deferred paste-line for $0 cost confirmation); honors D-24 cleanup-contract (CDK destroy first, terraform destroy second). RUNBOOK section: 3-step quy trinh + 24h Cost Explorer paste-line + ECR force_delete hint + CloudFront-don't-interrupt warning. Covers OBS-04 (verification), Phase 4 SC#4 + SC#5. *(3 commits; ZERO live AWS work — pure script + docs)*
