...
In scope: AgentCore Runtime resource (CDK Python), AgentCore execution IAM role + attachment of existing `hera-kb-retrieve-prod` policy (TF), ECR repo + 3-step build/push/deploy pipeline (TF + bin scripts), CloudWatch log group for AgentCore (TF), S3 widget hosting module (TF — public-read bucket, accessed via HTTPS S3 REST endpoint), Apple-Store-light branded widget polish (5 record-button states, color-coded transcript, WID-06 error states), public demo banner with $5/day cost cap copy. Out of scope: CloudFront / CDN / edge caching (rejected per D-26 update — S3 REST endpoint HTTPS is sufficient for instructor demo), CloudWatch dashboards/alarms (Phase 4 OBS-01..03), Lambda cost-circuit-breaker (Phase 4 OBS-05), `terraform destroy` cleanup verification script (Phase 4), workshop documentation chapters (Phase 5), custom domain + ACM cert.
...
- **D-25:** Update flow after first apply is **3 explicit paste-blocks**, not a 1-command wrapper and not CDK `DockerImageAsset`:
  ...
  3. `bin/deploy-widget.sh` — copies `frontend/` to `dist/widget/`, sed-replaces the AgentCore endpoint URL into `dist/widget/app.js`, then `aws s3 sync dist/widget/ s3://hera-widget-prod/`. No CloudFront invalidation (no CloudFront per D-26 revised).
...
### Widget hosting — S3 only (no CloudFront)
- **D-26 (revised 2026-05-05):** Widget HTML/JS hosts on **S3 only**, no CloudFront. AWS-native chosen over GitHub Pages so the workshop teaches "host static UI on S3" as part of the deliverable; CloudFront removed because the instructor-demo scope does not need edge caching, ACM, or origin-access protection — and CloudFront adds 2-3 modules of unnecessary complexity for the workshop teaching surface.
  - **HTTPS via S3 REST endpoint, NOT S3 static-website-hosting:** `getUserMedia` requires HTTPS (or localhost) — the legacy "S3 static website hosting" feature serves over HTTP only and would BREAK mic capture. Use the **S3 REST endpoint** (`https://<bucket>.s3.<region>.amazonaws.com/<key>`) which AWS provides with a service-managed TLS cert. The instructor demo URL is therefore `https://hera-widget-prod.s3.ap-northeast-1.amazonaws.com/index.html`.
  - **Bucket config:** public-read at object level (Block Public Access OFF for ACL/policy on this bucket only; `aws_s3_bucket_public_access_block` allows public ACL + policy). Bucket policy grants `s3:GetObject` to `Principal: "*"` scoped to `arn:aws:s3:::hera-widget-prod/*` — zero wildcards in Action, scoped Resource (D-13 still satisfied: Action explicit, Resource bound to this bucket).
  - **Trade-offs accepted:** URL aesthetic is `*.s3.ap-northeast-1.amazonaws.com/index.html` (no custom domain); no edge cache (irrelevant for demo); no auto-index document (caller must include `/index.html` explicitly); no error-document fallback (any 404 returns the S3 native XML — acceptable since there is one HTML page only).
  - **New Terraform module `infra/modules/widget_hosting/`** ships: S3 bucket (versioning enabled for rollback), public-access-block override, bucket policy granting `s3:GetObject` on `arn:aws:s3:::hera-widget-prod/*`. No CloudFront, no OAC, no ACM. Output `widget_url` = `https://hera-widget-prod.s3.ap-northeast-1.amazonaws.com/index.html` (constructed in TF locals).

### Endpoint URL injection — build-time sed-replace
- **D-27 (revised 2026-05-05):** Widget knows the AgentCore WSS URL via **build-time sed-replace into a build copy of `frontend/`**, not runtime fetch and not query string. `bin/deploy-widget.sh` copies `frontend/` to `dist/widget/`, reads `terraform output -raw agentcore_wss_url`, runs `sed -i "s|__AGENTCORE_WSS_URL__|${AGENTCORE_WSS_URL}|g" dist/widget/app.js`, then `aws s3 sync dist/widget/ s3://hera-widget-prod/`. NO CloudFront cache invalidation (no CloudFront — D-26 revised); browser cache is the only stale-asset risk and is mitigated by S3 versioning + visible-version timestamp in the page. Existing `frontend/app.js:12` constant `WS_URL = "ws://localhost:8080/ws"` becomes `WS_URL = "__AGENTCORE_WSS_URL__"` with a default-fallback for local dev. Reasoning: simplest possible, no runtime config-fetch round trip, no error path for missing config.json, FCJ paste-friendly.
  - **Local dev unchanged:** `docker compose up` continues to work against `ws://localhost:8080/ws`. The sed-replace targets only the build copy under `dist/widget/`, not the working tree.
...
- `infra/envs/prod/outputs.tf` — Phase 3 adds: `agentcore_wss_url`, `ecr_repo_url`, `agentcore_role_arn`, `agentcore_log_group_arn`, `widget_url` (full HTTPS S3 REST URL), `widget_s3_bucket_name`.
...
- **TF root + thin module** (Phase 1 D-08 single-module shape): `infra/envs/prod/main.tf` is thin; modules under `infra/modules/<name>/` follow `versions.tf / variables.tf / main.tf / outputs.tf` four-file shape. Phase 3 adds `agentcore_iam`, `widget_hosting` (S3-only), `ecr` modules in the same shape.
- **Zero IAM wildcards** (D-13): every Action and Resource is explicitly typed. Phase 3 AgentCore exec role policy: `bedrock:InvokeModelWithBidirectionalStream` scoped to the Nova 2 Sonic model ARN, `bedrock:Retrieve` already scoped via the attached `hera-kb-retrieve-prod` policy, CloudWatch logs/metrics scoped to the Phase 3 log group ARN. Widget bucket policy: `s3:GetObject` (single Action) on `arn:aws:s3:::hera-widget-prod/*` (scoped Resource) for `Principal: *` — public-read by design, NOT a wildcard violation.
...
- **Widget → AgentCore:** WSS URL injected into a build copy (`dist/widget/app.js`) at deploy time via sed-replace from `terraform output -raw agentcore_wss_url`. CORS configured on AgentCore endpoint (researcher confirms how) to allow the S3 origin (`https://hera-widget-prod.s3.ap-northeast-1.amazonaws.com`).
...
- **Demo URL shape:** `https://hera-widget-prod.s3.ap-northeast-1.amazonaws.com/index.html` (S3 REST endpoint HTTPS).
...
- **CloudFront / CDN / edge caching** — rejected per D-26 revision. Instructor demo does not need edge cache, ACM, or origin-access protection. CloudFront would add 2-3 unnecessary modules and a CloudFront cache-invalidation step in the deploy script. If a future v2 needs CDN (e.g., production-grade traffic, custom domain), add a CloudFront module on top of the existing S3 bucket — bucket can stay public-read or be flipped to OAC at that point.
- **Custom domain + ACM cert** — defer to v2. S3 REST endpoint URL aesthetic (`hera-widget-prod.s3.ap-northeast-1.amazonaws.com/index.html`) is acceptable for instructor demo. Adding a real domain requires either CloudFront (rejected) or Route53-aliased S3 website endpoint (HTTP only — breaks `getUserMedia`). Both routes are v2 work.
...
2. Public HTTPS demo URL on `https://hera-widget-prod.s3.ap-northeast-1.amazonaws.com/index.html` opens, mic permission grants, WSS to AgentCore endpoint connects, voice loop with at least one KB-backed answer ("Do you have MacBook Pro?" → audible Apple Store reply) plays through speakers, transcript visible.
