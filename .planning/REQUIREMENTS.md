...
- [~] **AWS-NAT-01..05**: DROPPED 2026-05-16 — user decision after live deploy + Lex test feedback: keep only the v1 Nova 2 Sonic web widget voice path; drop the Phase 6.1 Amazon Connect / Lex V2 / Polly Neural multi-component stack. Rationale: Nova 2 Sonic is the newer technology (1 foundation model end-to-end speech-to-speech with barge-in / sub-2s latency), whereas the Lex V2 + Polly pipeline is older IVR-style architecture (multi-hop ASR → intent → Lambda → TTS with 3-5s gaps). Phone number was also blocked at AWS new-account eligibility level (would have required 1-2 business day AWS Support ticket). Live AWS state torn down via `terraform destroy -target=module.aws_voice_channel` on 2026-05-16; Phase 6.1 module deleted from repo. Phase 6 PARTIAL Twilio archive (`infra/archive/twilio_bridge_phase6_partial/`) preserved unchanged.
- [x] **AWS-NAT-06**: Phase 6 leftover cleanup absorbed into Phase 6.1 Plan 01 — `terraform destroy -target=module.twilio_bridge` + Secrets Manager `force-delete-without-recovery` for `hera/twilio/auth-token` + `infra/envs/prod/main.tf` removes the `module "twilio_bridge"` block + `git mv infra/modules/twilio_bridge/ infra/archive/twilio_bridge_phase6_partial/` with a new README explaining D-56 (D-71). Acceptance: `aws apprunner list-services`, `aws ecr describe-repositories --repository-names hera-twilio-bridge`, `aws iam get-role --role-name hera-twilio-bridge-{instance,access}-prod`, `aws logs describe-log-groups --log-group-name-prefix /aws/apprunner/hera-twilio-bridge-prod`, and `aws secretsmanager describe-secret --secret-id hera/twilio/auth-token` all return Resource-Not-Found / count=0. *(live-verified Plan 06.1-01, 2026-05-16 — 5 Phase 6 leftover AWS resources confirmed gone via 6 CLI checks + 2 nested App Runner log groups manually cleaned; `infra/modules/twilio_bridge/` archived under `infra/archive/twilio_bridge_phase6_partial/` per D-71 with 20 git-mv file renames. AWS-NAT-06 stays VALID even after AWS-NAT-01..05 drop because it covers cleanup of the original Phase 6 Twilio attempt, not the Phase 6.1 Connect attempt.)*
...
| AWS-NAT-01 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DROPPED 2026-05-16 (user kept v1 Nova Sonic; Phase 6.1 stack destroyed) |
| AWS-NAT-02 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DROPPED 2026-05-16 |
| AWS-NAT-03 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DROPPED 2026-05-16 |
| AWS-NAT-04 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DROPPED 2026-05-16 |
| AWS-NAT-05 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DROPPED 2026-05-16 |
| AWS-NAT-06 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | Complete (Plan 06.1-01 — Twilio Phase 6 leftover cleanup; SURVIVES the AWS-NAT-01..05 drop) |
