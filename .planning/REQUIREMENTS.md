...
- [~] **AWS-NAT-01**: Amazon Connect instance `hera-voice-prod` in `us-east-1` with 1 claimed US DID phone number, configured by Terraform. Acceptance: `aws connect list-instances --region us-east-1` returns 1 instance + `aws connect list-phone-numbers-v2 --target-arn <instance-arn>` returns ≥ 1 US DID. *(PARTIAL — Plan 06.1-03, 2026-05-16: Connect instance live `arn:aws:connect:us-east-1:851725411875:instance/2f9fbb50-4526-423a-8180-b9ec2cff6d96`; phone number DEFERRED due to AWS new-account eligibility blocker — both API and Console claim returned false-positive "limit exceeded" with 0 phones actually claimed account-wide. Unblock via AWS Support ticket; see `06.1-03-SUMMARY.md` + RUNBOOK Phase 6.1 "Phone number blocker" section.)*
- [x] **AWS-NAT-02**: Lex V2 bot `hera-product-lookup-prod` (us-east-1) wired to an `AMAZON.FallbackIntent` that always invokes the Lookup Lambda, capturing the caller's spoken transcript via `event.inputTranscript` (D-68). Acceptance: `aws lexv2-models list-bots --region us-east-1` returns the bot; intent has Lambda fulfillment configured. *(live-verified Plan 06.1-03, 2026-05-16 — bot `EP8MNSCULA` alias `prod` id `Y6L5UXBXFQ`; FallbackIntent `FALLBCKINT` imported from Lex V2 auto-create; aws_lambda_permission.lex_invoke wires the lookup Lambda)*
- [x] **AWS-NAT-03**: Lookup Lambda `hera-voice-lookup-prod` (Python, us-east-1) calls Bedrock KB `Retrieve` cross-region (us-east-1 → ap-northeast-1) against `BKXE19AH89` and returns a one-sentence answer in the Lex V2 fulfillment response shape. Acceptance: synthetic invoke with slot text "iPhone 13 Pro Max in stock?" returns a payload containing "iPhone 13 Pro Max" + one of {"in stock", "available", "out of stock", "stock"}. *(live-verified Plan 06.1-03, 2026-05-16 — synthetic `aws lambda invoke` returned `sessionAttributes.answer = "Yes, Do you have iPhone 13 Pro Max in stock? is available."` containing "iPhone 13 Pro Max" + "available"; via boto3 bedrock-agent-runtime:Retrieve cross-region against `BKXE19AH89` in ap-northeast-1; D-70 attachment to existing `hera-kb-retrieve-prod` policy)*
- [x] **AWS-NAT-04**: Connect Contact Flow `hera-voice-flow-prod` greets the caller (Polly Neural Joanna en-US), transfers to the Lex bot, and reads back `$.Lex.SessionAttributes.answer` (set by the Lex Lambda response per D-69 addendum — see `06.1-CONTEXT.md` D-69 addendum + `06.1-RESEARCH.md` Open Question 1 for rationale). D-69 ~6-block shape: Set Voice → Play Prompt greeting → Get Customer Input → Play Prompt response → Disconnect, plus single-attempt error branch. Acceptance: `aws connect describe-contact-flow` returns `Status=PUBLISHED`, the JSON definition references the Lex bot alias ARN + Polly Neural Joanna voice + the literal `$.Lex.SessionAttributes.answer`, and the flow ARN is attached to the claimed DID. *(live-verified Plan 06.1-03, 2026-05-16 — contact_flow_id `bcc2bf50-55f5-4329-8f3f-573844c7aa11` PUBLISHED; JSON contains `Joanna` + `neural` + `$.Lex.SessionAttributes.answer` + Lex bot alias ARN; phone-attach deferred pending AWS-NAT-01 unblock; Contact Flow schema fix required 3 error transitions in `ConnectParticipantWithLexBot`: InputTimeLimitExceeded + NoMatchingCondition + NoMatchingError)*
- [~] **AWS-NAT-05**: CCP browser-softphone smoke verification — operator opens the Connect CCP, dials the US DID, asks "Do you have iPhone 13 Pro Max in stock?" via mic, and hears a Polly response containing both "iPhone 13 Pro Max" and a stock-status word within 5 seconds of end-of-question. Acceptance: PASS self-reported in `06.1-HUMAN-UAT.md`; FAIL paths for >5s latency or wrong answer trigger gap closure. *(DEFERRED Plan 06.1-03, 2026-05-16 — gated on AWS-NAT-01 phone number unblock. `06.1-HUMAN-UAT.md` template ready for operator self-report once phone is claimed.)*
- [x] **AWS-NAT-06**: Phase 6 leftover cleanup absorbed into Phase 6.1 — `terraform destroy -target=module.twilio_bridge` + Secrets Manager `force-delete-without-recovery` for `hera/twilio/auth-token` + `infra/envs/prod/main.tf` removes the `module "twilio_bridge"` block + `git mv infra/modules/twilio_bridge/ infra/archive/twilio_bridge_phase6_partial/` with a new README explaining D-56 (D-71). Acceptance: `aws apprunner list-services`, `aws ecr describe-repositories --repository-names hera-twilio-bridge`, `aws iam get-role --role-name hera-twilio-bridge-{instance,access}-prod`, `aws logs describe-log-groups --log-group-name-prefix /aws/apprunner/hera-twilio-bridge-prod`, and `aws secretsmanager describe-secret --secret-id hera/twilio/auth-token` all return Resource-Not-Found / count=0. *(live-verified Plan 06.1-01 + 06.1-03, 2026-05-16 — 5 Phase 6 leftover AWS resources confirmed gone via 6 CLI checks + 2 nested App Runner log groups manually cleaned; `infra/modules/twilio_bridge/` archived under `infra/archive/twilio_bridge_phase6_partial/` per D-71 with 20 git-mv file renames; D-71 README written with 5 sections covering App Runner inbound-WS limitation + ECS Fargate re-pivot pointer)*
...
| AWS-NAT-01 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | PARTIAL (Plan 06.1-03 — instance live; phone deferred for AWS Support ticket) |
| AWS-NAT-02 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | Complete (Plan 06.1-03) |
| AWS-NAT-03 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | Complete (Plan 06.1-03) |
| AWS-NAT-04 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | Complete (Plan 06.1-03) |
| AWS-NAT-05 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | DEFERRED (Plan 06.1-03 — gated on AWS-NAT-01 phone unblock) |
| AWS-NAT-06 | Phase 6.1 — Native AWS Voice Channel — Amazon Connect | Complete (Plan 06.1-01 + 06.1-03) |
